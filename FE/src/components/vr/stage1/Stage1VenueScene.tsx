...
import {
  STAGE1_STAGE_CENTER,
  STAGE1_STAGE_LOOK_AT,
  stage1SeatViewPosition,
} from '../../../utils/seatMap3DStage1';
import { DesktopYawControl } from '../DesktopYawControl';
import { VenueModel } from '../VenueModel';
import { Stage1SeatInstances } from './Stage1SeatInstances';

interface Stage1VenueSceneProps {
  seats: Seat3D[];
...
function VrFloorGuide({ spawn }: { spawn: VrSpawn }) {
  ...
  return (
    <group position={[spawn.floorCenter[0], spawn.floorY, spawn.floorCenter[2]]}>
      ...
      <gridHelper args={[spawn.floorW, 28, '#475569', '#1e293b']} position={[0, 0.02, 0]} />
    </group>
  );
}
...
function FallbackStage() {
  return (
    <group position={STAGE1_STAGE_CENTER}>
      <mesh receiveShadow>
...
function SeatViewCamera({
  ...
  const lookAt = useRef(new Vector3(...STAGE1_STAGE_LOOK_AT));

  useEffect(() => {
    if (seat && active) {
      const [x, y, z] = stage1SeatViewPosition(seat);
...
export function Stage1VenueScene({
  seats,
  selectedIds,
  previewSeatId,
  viewFromSeat,
  ...
  return (
    <>
      ...
      <directionalLight position={[5, 14, 8]} intensity={1.4} castShadow={!xrSession} />
      <directionalLight position={[-4, 8, -2]} intensity={0.5} color="#c4b5fd" />
